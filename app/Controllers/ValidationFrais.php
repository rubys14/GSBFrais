...
class ValidationFrais extends BaseController
{
    ...
    {
        $this->nom_prenom = $this->request->getPost('lstvisi');
        return $this->commun();
    }
    ...
    private function commun()
    {
        ...
        if (count($les_anneemois) == 0) {
            return redirect()->back()->withInput()->with('erreurs', "Aucune fiche de frais n'a été saisie pour cet utilisateur");
        } else {
            ...
            $data['lst_id'] = 'lstMois';
            $data['lst_label'] = 'Mois';
            $data['sc_titre'] = 'Visiteur et Mois à sélectionner :';


            $dataUser['lst_contenu'] = $options;
            $dataUser['lst_select'] = $this->nom_prenom;
            $dataUser['lst_action'] = 'etatfrais/mois';
            $dataUser['lst_id'] = 'lstvisi';
            $dataUser['lst_label'] = 'Visiteur';
            $dataUser['sc_titre'] = '';
            ...
            echo view('liste_deroulante', $dataUser);
